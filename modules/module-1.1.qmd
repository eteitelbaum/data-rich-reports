...
- Start a new quarto project called "module-1.1" and generate a quarto document by the same name so that you can use to follow along with this exercise
...
In this module, we will be using the `readr`, `tidyr` and `dplyr` from the Tidyverse. While we could just load the entire Tidyverse collection of packages (e.g. `library(tidyverse)`), it is generally better from a [performance standpoint](https://stackoverflow.com/questions/50257990/what-is-the-advantage-of-using-librarytidyverse-instead-of-sub-packages) to load the packages individually. 
...
## Step 1: Download data from the World Bank
...
## Step 4: Fix year and change variables to numeric
