...
Familiarize yourself with this setup code and the dataframes it generates. 
...
flfp_region <- ggplot(flfp_gdp_regions, aes(x = reorder(region, -flfp), y = flfp, fill = region)) +
  geom_col() + 
...
```{r}
flfp_region + scale_fill_brewer(palette = "BrBG")
```


## Scale color for scatterplots an line charts
...
```{r}
flfp_region + scale_fill_brewer(palette = "BrBG") + theme_minimal()
```
...
```{r}
flfp_line <- flfp_line + scale_color_brewer(palette = "RdYlBu") 
...
```{r}
flfp_region <- flfp_region + scale_fill_brewer(palette = "BrBG") + theme_minimal()
