...
Let's go ahead and apply our three new verbs to the `dem_women` data frame and store the resulting new data frame in an object called `dem_summary`. Then we will print the object to view its contents. 
...
Run the code and try changing it yourself. Try summarizing the data with a different function for one or more of the variables. Now try grouping by country instead of region. Finally try arranging the data in ascending rather than descending order (hint: delete "desc" from the `arrange()` call).  
