...
[leaflet](http://rstudio.github.io/leaflet/), [simple features](https://r-spatial.github.io/sf/) and [html tools](https://rstudio.github.io/htmltools/) and have a look at the documentation for each.
...
This module is going to introduce you to pop-up maps. Pop-ups on maps are essentially windows that allow you to show information related to a particular point or feature on a map without having to navigate away from the current view.
Pop-ups can be used to display information such as an address or the name of a city or town. You can customize how pop-ups look, choose what data to display in them and, as you get more advanced, you can even link them to external websites. 

## Working with UCDP data

{{< video https://www.youtube.com/embed/wo9vZccmqwc title = 'Working with UCDP data'>}}

Our running example in this module is going to involve mapping conflict events for Yemen from the Uppsala Conflict Data Program [UCDP](https://ucdp.uu.se/). So while we will be building on some of our earlier knowledge, this module is going to depart a bit from previous ones in that we are using an entirely new dataset. Specifically, we will be using the UCDP georeferenced event dataset, which you can download from[here](https://ucdp.uu.se/downloads/index.html#ged_global). 

After you have downloaded the data and saved it in your modules folder, go ahead and, read it in and have a look at its contenst with `gplimpse()`. 
...
{{< video https://www.youtube.com/embed/wo9vZccmqwc title = 'Plot conlfict'>}}

Now that we have our data, let's make our first pop-up map. To do this we are going to be using the [leaflet](https://rstudio.github.io/leaflet/) package. Leaflet is a really popular JavaScript library for interactive mpas. To get started with leaflet, we'll make a really simple map with one marker that says "First conflict event." 
...
Now that you have the hang of it, we can move on to plotting some conflict events from our data frame. Again we will call `leaflet()` but this time we will add `data = ged_yemen` as an argument. We will also use `setView()` to center the map on Yemen's capital Sana'a. We include two arguments for `addMarkers`. `popup = ~as.character(deaths)` displays the number of deaths when the user clicks on the marker and `label = ~where_coordinates` displays the name of the town that the coordinates correspond to.  
...
leaflet(data = ged_yemen) |> # map points in ged_yemen data frame
  ...
  addMarkers(
    ...
    label = ~where_coordinates # when user hovers, show town
    )
```

## Customize your leaflet plot

{{< video https://www.youtube.com/embed/wo9vZccmqwc title = 'Customize your leaflet plot'>}}
